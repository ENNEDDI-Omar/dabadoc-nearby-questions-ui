...
export class QuestionService {
    ...
    return this.http.get<any[]>(url, { params }).pipe(
        map(response => {
          ...
          return response.map(item => {
            const answersCount = typeof item.answers_count === 'number' ? item.answers_count :
                (item.answers_count === undefined ? 0 : parseInt(item.answers_count));
            return {
              id: item._id || item.id,
              _id: item._id,
              title: item.title,
              ...
              answers_count: item.answers_count !== undefined ? item.answers_count : 0,
              favorites_count: item.favorites_count !== undefined ? item.favorites_count : 0
            } as Question;
          });
        })
    );
  ...
  getFavorites(): Observable<Question[]> {
    ...
    return this.http.get<Question[]>(`${environment.apiUrl}/favorites`).pipe(
        map(response => {
          return response.map(item => {
            return {
              id: item._id || item.id,
